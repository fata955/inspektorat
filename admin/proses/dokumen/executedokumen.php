...
if ($_SESSION == null) {
  ...
  if ($_GET["action"] === "insertData") {
    ...
    if (in_array($fileType, $allowTypes)) {
      ...
      if (move_uploaded_file($_FILES["filegambar"]["tmp_name"], $targetFilePath)) {
        $query  = "INSERT INTO dokumen (nama_dokumen,tahun,status,dokumen) VALUES('$judul','$tahun','$status','$fileName')";
        ...
        echo json_encode([
          "statusCode" => 404,
          "message" => "Data Error Perhatikan data gambar anda"
        ]);
      }
    } else {
      echo json_encode([
  ...
  if ($_GET["action"] === "updateData") {
    ...
    $tahun = $_POST["tahun"];
    $status = $_POST["status"];
    $data = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM dokumen where id=$id"));
    $gambar = $data['dokumen'];
    $targetDir = "../../doc/";
    $fileName = basename($_FILES["filegambar1"]["name"]);
    $targetFilePath = $targetDir . $fileName;
    $fileType = pathinfo($targetFilePath, PATHINFO_EXTENSION);


    $allowTypes = array('pdf');
    // if ($gambar != "") unlink("../../uploads/".$gambar);
    // $allowTypes = array('jpg', 'png', 'jpeg', 'gif');
    if (in_array($fileType, $allowTypes)) {
      //upload file to server
      if (move_uploaded_file($_FILES["filegambar1"]["tmp_name"], $targetFilePath)) {
        if ($gambar != "") unlink("../../doc/" . $gambar);
        $query  = "UPDATE dokumen SET nama_dokumen='$judul', tahun='$tahun', status='$status',dokumen='$fileName' where id=$id";
        $result = mysqli_query($koneksi, $query) or die(mysqli_error($koneksi));
        echo json_encode([
          "statusCode" => 200,
          "message" => "Data " . $fileName . " Berhasil tersimpan"
        ]);
        // $statusMsg = "The file " . $fileName . " has been uploaded.";
      } else {
        echo json_encode([
  ...
  if ($_GET["action"] === "deleteData") {
    ...
    $data = mysqli_fetch_array(mysqli_query($koneksi, "select * from dokumen where id=$id"));
    if ($data['dokumen'] != "") unlink("../../doc/" . $data['dokumen']);
